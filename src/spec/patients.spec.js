import {AngularJSSelector, AngularSelector, waitForAngular} from "testcafe-angular-selectors";
...
test.only("Verify Patients List", async () => {
  await t.maximizeWindow();
  await t.switchToMainWindow();
  ...
  await waitForAngular();
  await t.switchToMainWindow();
  const patProfile = await Selector("span").withExactText("Patient: BEAUVAIS, Robert").filterVisible();
  await t.hover(patProfile);
  await t.click(".pencil-icon");
  await t.setNativeDialogHandler(() => true);
  const updateMenu = await Selector("div.modal-content div.text-center.h5").innerText;
